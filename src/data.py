...
def load(args):
    ...
    srcDir = os.path.dirname(current)
    projectDir = os.path.dirname(srcDir)
    file = os.path.join(projectDir, "datasets", args.data, args.dataset, "splits", str(args.split) + ".pickle")

    if not os.path.isfile(file): print("split + ", str(args.split), "does not exist")
    with open(file, 'rb') as H:
        Splits = pickle.load(H)
        train, test = Splits['train'], Splits['test']  # key가 train 인 부분, test인 부분을 각가 나누어 할당
...
class parser(object):
    ...
    def __init__(self, data, dataset):
        ...
        projectDir = os.path.dirname(current)
        self.d = os.path.join(projectDir, "datasets", data, dataset)  # d 변수 추가
